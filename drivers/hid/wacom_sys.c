...
static void wacom_remote_destroy_battery(struct wacom *wacom, int index)
{
	...
	if (remote->remotes[index].battery.battery) {
		...
		remote->remotes[index].active_time.tv64 = 0;
	}
}
...
static int wacom_remote_attach_battery(struct wacom *wacom, int index)
{
	...
	if (!remote->remotes[index].active_time.tv64)
		return 0;
...
static void wacom_remote_work(struct work_struct *work)
{
	...
	for (i = 0; i < WACOM_MAX_REMOTES; i++) {
		...
		if (data.remote[i].connected) {
			...
			if (kt.tv64 - remote->remotes[i].active_time.tv64 > WACOM_REMOTE_BATTERY_TIMEOUT
			    && remote->remotes[i].active_time.tv64 != 0)
				wacom_remote_destroy_battery(wacom, i);
